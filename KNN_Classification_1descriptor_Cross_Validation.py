# -*- coding: utf-8 -*-
"""
Created on Sat Mar  2 22:12:43 2019
...
plt.title("Confusion matrix Sum KNN descriptor (" +  Name_descriptors[Number_descriptor] +  ") Not normalized")
sn.heatmap(results_sum, annot=True, cmap="YlGnBu", linewidths=5.0)

plt.figure(figsize = (10,10))
plt.title("Confusion matrix Sum KNN descriptor (" +  Name_descriptors[Number_descriptor] +  ") normalized")
...
for idx in indice_set:
    ...
    x_set_norm[compt,:] = descriptor_df_norm.get(Name_descriptors[Number_descriptor]).loc[index_img][1:]
    x_set[compt,:] = descriptor_df.get(Name_descriptors[Number_descriptor]).loc[index_img][1:]
    compt = compt + 1
